...
- **Widget interactions that don't need the server stay on the client.** Sorting, filtering, scrolling, and selection highlighting are client-local — no round-trip. Actions that change app state (navigate, delete, submit) go to the server and take a round-trip. This is the web model: local interactions are instant, server interactions take ~200ms. See [`docs/INPUT_LATENCY.md`](docs/INPUT_LATENCY.md).
